		...
		if g:EasyMotion_do_mapping
			for [motion, fn] in items(a:motions)
				if g:EasyMotion_special_{fn.flag}
					silent exec 'onoremap <silent> ' . motion . ' :call EasyMotion#' . fn.name . '()<CR>'
					silent exec 'nnoremap <silent> v' . motion . ' :call EasyMotion#' . fn.name . '()<CR>'
					silent exec 'nnoremap <silent> y' . motion . ' :call EasyMotion#' . fn.name . 'Yank()<CR>'
					silent exec 'nnoremap <silent> d' . motion . ' :call EasyMotion#' . fn.name . 'Delete()<CR>'
				endif
			endfor
		endif
	endfunction "}}}
